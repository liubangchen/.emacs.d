...
;; Author: chenlong
;; Description: 通过 claude CLI 与 CodeBuddy 交互的 Emacs 模块
;; 架构: 三窗口布局
...
(defun codebuddy--ensure-vterm ()
  "确保 vterm 已加载，懒加载以避免启动时硬依赖."
  (unless (featurep 'vterm)
    ...
    (unless (locate-library "vterm")
      (let ((vterm-dir (car (file-expand-wildcards
                             (expand-file-name "elpa/vterm-*" user-emacs-directory)))))
        (when (and vterm-dir (file-directory-p vterm-dir))
...
(defcustom codebuddy-executable nil
  "claude 可执行文件路径.
nil 时自动检测: 优先 claude，fallback claude-internal."
  :type '(choice (const :tag "自动检测" nil) string)
  :group 'codebuddy)

(defcustom codebuddy-default-args '("-p" "--dangerously-skip-permissions"
...
(defun codebuddy--resolve-executable ()
  "返回实际使用的 claude 可执行文件路径.
优先使用 `codebuddy-executable' 自定义值，
否则自动检测: claude → claude-internal."
  (or codebuddy-executable
      (executable-find "claude")
      (executable-find "claude-internal")
      (error "CodeBuddy: 未找到 claude 或 claude-internal 命令行工具")))
...
(defun codebuddy--dispatch-line (line)
  "解析一行 stream-json 输出 LINE 并分发到对应 buffer.
每行是一个独立的 JSON 对象，根据 type 字段分发:
- system: 初始化信息 → thinking buffer
- assistant: 思考/结果内容 → thinking/result buffer
- result: 完成统计 → thinking buffer"
  (let ((trimmed (string-trim line)))
    (when (and (not (string-empty-p trimmed))
               (eq (aref trimmed 0) ?{))  ; 快速跳过非 JSON 行
      (condition-case err
          (let* ((json (json-parse-string trimmed :object-type 'alist))
            ...
            (cond
             ;; system/init: 提取模型信息
             ((string= type "system")
              (let ((model (alist-get 'model json))
                    (subtype (alist-get 'subtype json)))
                (codebuddy--append-to-thinking
                 (format "\n* System (%s)\nModel: %s\n"
                         (or subtype "info") (or model "unknown")))))

             ;; assistant: 遍历 message.content[]
             ((string= type "assistant")
              (let* ((message (alist-get 'message json))
                     (content (alist-get 'content message)))
                (when (vectorp content)
                  (seq-doseq (block content)
                    (let ((content-type (alist-get 'type block)))
                      (cond
                       ;; thinking 块 → thinking buffer
                       ...
                       ;; text 块 → result buffer
                       ((string= content-type "text")
                        (let ((text (alist-get 'text block)))
                          (when (and text (not (string-empty-p text)))
                            (setq codebuddy--current-response
                                  (concat codebuddy--current-response text))
                            (codebuddy--append-to-result text)
                            (codebuddy--append-to-thinking
                             "\n* Result (→ Result Buffer)\n"))))

                       ;; tool_use 块 → thinking buffer
             ...
             ((string= type "result")
              (let* ((usage (alist-get 'usage json))
                     ...
                     (input-tokens (and usage (alist-get 'input_tokens usage)))
                     (output-tokens (and usage (alist-get 'output_tokens usage)))
                     (total (and input-tokens output-tokens
                                 (+ input-tokens output-tokens))))
                (when total
                  (setq codebuddy--last-tokens-used total))
                (codebuddy--append-to-thinking
                 (format "\n* Tokens Used\ninput: %s  output: %s  total: %s  cost: $%.4f\n"
                         (if input-tokens (codebuddy--format-number input-tokens) "?")
                         (if output-tokens (codebuddy--format-number output-tokens) "?")
                         (if total (codebuddy--format-number total) "?")
                         (or cost 0)))))

             ;; 其他类型 → thinking buffer
             (t
              (codebuddy--append-to-thinking
               (format "\n[%s] %s\n" (or type "unknown")
                       (truncate-string-to-width trimmed 200 nil nil "..."))))))
        ;; JSON 解析失败：作为纯文本输出到 result buffer（兼容非 JSON 输出）
        (error
         (unless (string-empty-p trimmed)
           (codebuddy--append-to-result (concat trimmed "\n"))))))))

(defun codebuddy--process-filter (_proc output)
  "处理 claude CLI 的 stream-json 输出 OUTPUT.
正确处理跨批次的不完整行."
  ;; 拼接上次残余
...
由 vterm escape sequence 触发."
  ...
  (if (and (string-prefix-p "/" input)
    ...
    (setq codebuddy--last-tokens-used nil)
    ;; 构造带上下文的 prompt
    (let* ((exe (codebuddy--resolve-executable))
           (context-prompt (if codebuddy--db
                               (codebuddy--build-context-prompt input)
                             input))
           (args (append codebuddy-default-args
                         (list context-prompt)))
           (default-directory codebuddy--work-directory)
    ...
    (if (and (buffer-live-p buf)
      ...
      (with-current-buffer buf
        (let ((vterm-shell (format "/bin/zsh -c '%s'" codebuddy-input-script))
          ...
          ;; 记录 vterm 内部的 shell 进程，并添加 sentinel 清理
          (let ((proc (get-buffer-process buf)))
            (setq codebuddy--vterm-process proc)
            (when proc
